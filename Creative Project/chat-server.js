...
let rooms = { Main: { users: [], messages: [], password: null, owner: "server"} };  // Initialize with a "Main" room
...
io.sockets.on("connection", function (socket) {
    ...
    socket.on('join_room', function(data) {
        ...
        if (rooms[roomToJoin] || roomToJoin === 'Main') {
            if (socket.prevRoom) {
    ...
    socket.on('create', function(room) {
        ...
        if (!rooms[roomName]) {         // if this room does not already exist
            rooms[roomName] = { users: [], messages: [], password: access === "private" ? pass : null, owner: host};  // Initialize with users and messages and password if applicable 
            ...
            io.sockets.emit("room_list", Object.keys(rooms).map(room => ({
                ...
                host: rooms[room].owner,
                blackList: rooms[room].blackList || []  // Include blackList, defaulting to an empty array if undefined
            })));  // Send list of rooms with privacy info to all clients
